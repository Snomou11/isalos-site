...
function AvailabilityManager({ setView, rooms }: { setView: (v: View) => void; rooms: Room[] }) {
  ...
  return (
    <div>
      ...
      {!selectedSlug && (
        <p className="text-gray-400 text-center py-16">← Select a room to manage its availability</p>
      )}
        ...
        <>
          <div className="flex items-center justify-between mb-4">
            <button onClick={() => setCurrentMonth(new Date(year, month - 1, 1))}
              className="px-3 py-1 rounded border border-gray-300 hover:bg-gray-100">← Prev</button>
            <span className="font-semibold text-lg">{MONTHS[month]} {year}</span>
            <button onClick={() => setCurrentMonth(new Date(year, month + 1, 1))}
              className="px-3 py-1 rounded border border-gray-300 hover:bg-gray-100">Next →</button>
          </div>
          <p className="text-sm text-gray-500 mb-3">
            {selecting ? '📅 Click an end date to complete the range' : 'Click a start date, then an end date'}
          </p>
...
function PricesManager({ setView, rooms }: { setView: (v: View) => void; rooms: Room[] }) {
  ...
  return (
    <div>
      ...
      {!selectedSlug && (
        <p className="text-gray-400 text-center py-16">← Select a room to manage its prices</p>
      )}
