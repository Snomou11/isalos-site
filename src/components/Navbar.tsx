        ...
        <div className="hidden lg:flex items-center gap-12">
          {navLinks.map(l => (
            <Link key={l.href} href={l.href}
              className="text-lg font-semibold text-white/90 hover:text-white transition-all duration-300 py-2 hover:scale-105">
          ...
          <div className="flex items-center gap-3 ml-8">
            ...
            {/* Book Now */}
            <Link href="/rooms"
              className="px-8 py-3 font-bold text-sm bg-white text-black rounded-full shadow-xl hover:shadow-2xl hover:scale-105 transition-all duration-300 tracking-wide">
              {lang === 'en' ? 'Book Now' : 'Κράτηση'}
            </Link>

            {/* Logout — only when admin is logged in */}
            {isAdmin && (
              <button
                onClick={handleLogout}
                className="flex items-center gap-1.5 px-4 py-2.5 text-sm font-semibold text-red-400 hover:text-white hover:bg-red-600 border border-red-400/50 hover:border-red-600 rounded-full transition-all duration-300">
        ...
        <div className="lg:hidden bg-black/95 backdrop-blur-xl border-t border-white/20 px-6 py-8 space-y-4">
          ...
          <div className="pt-6 border-t border-white/20 flex items-center gap-4">
            ...
            {isAdmin && (
              <button
                onClick={handleLogout}
                className="px-4 py-3 text-sm font-semibold text-red-400 hover:text-white hover:bg-red-600 border border-red-400/50 rounded-2xl transition-all">
                Logout
              </button>
            )}
          </div>
